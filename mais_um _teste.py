...
print("🦅 EXECUTANDO PROTOCOLO FINAL DE EXCELÊNCIA: GEOCANABIS BR...")

# 1. CARGA E PREPARAÇÃO DOS DADOS
try:
    # Carregando a base final higienizada
...
# 2. CONFIGURAÇÃO DO MAPA (CERCAMENTO TÁTICO)
# Travando o mapa apenas no território brasileiro
limites_br = [[-34.5, -74.5], [5.5, -34.0]]
mapa = folium.Map(
    location=[-14.2350, -51.9253], 
    zoom_start=4, 
    tiles='cartodb positron', 
    control_scale=True, # Escala será movida para a direita via CSS
    min_zoom=4, 
    max_bounds=True,
...
# 3. RESTAURAÇÃO DO POLÍGONO DA MACONHA (SERTÃO)
coords_poligono = [
...
# 4. DEFINIÇÃO DE ATIVOS VISUAIS (ÍCONES E INSÍGNIAS)
icon_leaf = "https://img.icons8.com/color/48/marijuana-leaf.png"
icon_pistol_file = "image_9034be.png" # Arquivo de pistola fornecido pelo usuário
...
def get_badge(row):
    ...
    # Lógica Estadual (Exemplo: MG e PE)
    ...
    if 'PERNAMBUCO' in uf or 'PE' in uf:
        return "https://upload.wikimedia.org/wikipedia/commons/thumb/3/30/Bras%C3%A3o_da_PMPE.png/100px-Bras%C3%A3o_da_PMPE.png"
...
for _, row in df_mapa.iterrows():
    # Decisão de Ícone de Marcador: Prioridade Pistola se houver arma
    tem_arma = pd.notna(row['armas apreendidas']) and str(row['armas apreendidas']).strip() not in ['0', 'X', 'NÃO', '']
    icon_final = folium.CustomIcon(icon_pistol_file if tem_arma else icon_leaf, icon_size=(30, 30))
    ...
    <div style="width: 200px; font-family: Arial, sans-serif; font-size: 12px; text-align: center;">
        ...
        <div style="text-align: left; padding-left: 10px;">
            ...
            <img src="{icon_pistol_file}" width="14"> <b>Armas:</b> {row['armas apreendidas']}
        </div>
        <hr>
...
# 6. INTERFACE DE COMANDO (TÍTULO, LEGENDA, CRÉDITOS, NORTE)
# CSS para mover a escala para a direita e organizar as caixas
layout_html = f"""
    ...
    <div style="position: fixed; bottom: 50px; right: 20px; z-index: 9999; background: white; padding: 12px; border: 2px solid black; font-family: sans-serif; font-size: 11px;">
        ...
        <img src="{icon_pistol_file}" width="15"> = Arma de Fogo (Poder de Fogo)<br>
        <img src="{icon_leaf}" width="15"> = Cannabis (Local e Quantidade)<br>
...
print("\n✅ OPERAÇÃO CONCLUÍDA: 'GEOCANNABIS_BR_DEFINITIVO.html'")