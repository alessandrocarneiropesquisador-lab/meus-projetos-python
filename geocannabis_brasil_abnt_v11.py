...
# 2. CERCAMENTO TÁTICO (MAPA TRAVADO NO BRASIL)
...
# 4. ICONOGRAFIA PADRONIZADA (FOLHA)
...
# 5. MARCADORES COM CARDS ANTI-ESTOURAMENTO (ARIAL)
for _, row in df_mapa.iterrows():
    html_card = f"""
    <div style="width: 210px; max-height: 150px; overflow-y: auto; font-family: Arial, sans-serif; font-size: 11px; padding: 8px; line-height: 1.4;">
        <b style="color: #1B5E20; font-size: 13px;">📍 {row['municipio_ibge']} - {row['estado_ibge']}</b><hr style="margin: 5px 0;">
        ...
        <a href="{row['Link de acesso']}" target="_blank" style="color: #2980B9; text-decoration: none; font-weight: bold;">[Notícia Completa]</a>
    </div>
    """
    folium.Marker(
...
# 6. INTERFACE DE COMANDO (ABNT: ESCALA E LEGENDA À DIREITA)
layout_final = f"""
    <style>
        /* CSS REFORÇADO: MOVE A ESCALA PARA O LADO OPOSTO (DIREITA) */
    ...
    <div style="position: fixed; top: 15px; left: 50%; transform: translateX(-50%); z-index: 9999;">
        <h1 style="font-family: Arial, sans-serif; font-size: 12px; font-weight: bold; background: white; padding: 5px 15px; border: 1px solid #ccc; border-radius: 4px;">GEOCANABIS BR</h1>
    </div>
    ...
    <div style="position: fixed; bottom: 20px; left: 20px; z-index: 9999; font-family: Arial, sans-serif; font-size: 10px; line-height: 0.8; color: black; background: rgba(255,255,255,0.7); padding: 5px; border-radius: 3px;">
...
mapa.save('GEOCANABIS_BR_CIENTIFICO_FINAL.html')
